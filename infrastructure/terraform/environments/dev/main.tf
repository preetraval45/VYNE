# ============================================================
# VYNE — Dev Environment
# Composes all modules with dev-appropriate sizing
# ============================================================
...
terraform {
  ...
  backend "s3" {
    ...
    key            = "vyne/dev/terraform.tfstate"
    region         = "us-east-1"
    encrypt        = true
...
module "s3" {
  ...
  cors_allowed_origins = ["http://localhost:3000", "https://dev.${var.app_domain}"]
...
module "cognito" {
  ...
  environment   = var.environment
  callback_urls = ["http://localhost:3000/api/auth/callback/cognito", "https://dev.${var.app_domain}/api/auth/callback/cognito"]
  logout_urls   = ["http://localhost:3000", "https://dev.${var.app_domain}"]
}
