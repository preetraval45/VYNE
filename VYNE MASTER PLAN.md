...
- [x] **Forecasting** — _`ForecastingTab` now shows scenario categories (Closed Won / Commit / Forecast / Best Case), an editable+persisted **quota attainment bar** (closed + committed), pipeline-by-stage, win rate, and top deals._
- [x] **Reports & dashboards** — _Forecast/Reports tab: pipeline-by-stage, win rate, **conversion funnel** (step-conversion %), **pipeline-by-owner**, top deals + the CRM dashboard view._ (Scheduled-email reports deferred — needs the D2 email integration.)
- [~] **Automation workflows** — _Built a real, production-grade CRM rules engine: Prisma `AutomationRule` model + tenant-scoped `/api/automations` CRUD + store (mirror/hydrate) + a **Settings → Automations** builder ("When deal → {stage}, {action}"). Executes inside `updateDeal`: **log a note**, **set next action**, **set probability** — all in-app + fully functional, recorded on the deal timeline._ Still to do: more actions (create task / notify channel / send email) + generalize to the cross-module engine (**D3**).\_
