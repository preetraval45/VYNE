...
- [ ] **7.6** Density modes — Compact / Comfortable / Spacious toggle that scales `--row-height`, `--padding-card`, and `--font-size-base` via `data-density` on `<html>`. Persist on theme store.
...
- [ ] **7.10** Theme presets — bundle 6 curated themes (e.g. "Linear-dark teal", "Notion-light", "Salesforce-blue", "GitHub-dark", "Stripe-violet", "Solarized") as one-click apply that sets accent + font + density together.
