...
function ToolbarButton({
  ...
  return (
    <button
      ...
      onMouseEnter={(e) => {
        if (!active) (e.currentTarget as HTMLButtonElement).style.background = '#F8F8FC'
      }}
      onMouseLeave={(e) => {
...
// ── Format helpers ────────────────────────────────────────────────
function getSelectedText(textarea: HTMLTextAreaElement): { text: string; start: number; end: number } {
  return {
...
export function DocEditor({ doc }: DocEditorProps) {
  ...
  // ── Toolbar actions ───────────────────────────────────────────────
  function applyBold() {
    if (textareaRef.current) wrapSelection(textareaRef.current, setContent, '**', '**')
  }
  function applyItalic() {
    if (textareaRef.current) wrapSelection(textareaRef.current, setContent, '_', '_')
  }
  ...
  function applyOrdered() {
    if (textareaRef.current) prependLine(textareaRef.current, setContent, '1. ')
  }
  function applyQuote() {
    if (textareaRef.current) prependLine(textareaRef.current, setContent, '> ')
  }
  function applyDivider() {
    ...
    const hr = '\n---\n'
    const newValue = ta.value.substring(0, start) + hr + ta.value.substring(start)
    setContent(newValue)
    setTimeout(() => {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      >
        <ToolbarButton onClick={applyH1} title="Heading 1"><Heading1 size={14} /></ToolbarButton>
        <ToolbarButton onClick={applyH2} title="Heading 2"><Heading2 size={14} /></ToolbarButton>
        <ToolbarButton onClick={applyH3} title="Heading 3"><Heading3 size={14} /></ToolbarButton>
        <div className="w-px h-4 bg-[#E8E8F0] mx-1" />
        <ToolbarButton onClick={applyBold} title="Bold"><Bold size={14} /></ToolbarButton>
        <ToolbarButton onClick={applyItalic} title="Italic"><Italic size={14} /></ToolbarButton>
        <ToolbarButton onClick={applyStrike} title="Strikethrough"><Strikethrough size={14} /></ToolbarButton>
        <ToolbarButton onClick={applyCode} title="Inline code"><Code size={14} /></ToolbarButton>
        <div className="w-px h-4 bg-[#E8E8F0] mx-1" />
        <ToolbarButton onClick={applyBullet} title="Bullet list"><List size={14} /></ToolbarButton>
        <ToolbarButton onClick={applyOrdered} title="Ordered list"><ListOrdered size={14} /></ToolbarButton>
        <ToolbarButton onClick={applyQuote} title="Blockquote"><Quote size={14} /></ToolbarButton>
        <ToolbarButton onClick={applyDivider} title="Horizontal rule"><Minus size={14} /></ToolbarButton>
        {/* Save indicator */}
        <div className="ml-auto text-[11px]" style={{ color: '#A0A0B8' }}>
