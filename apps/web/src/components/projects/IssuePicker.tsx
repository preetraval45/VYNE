...
export function StatusPicker({ value, onChange, compact = false }: StatusPickerProps) {
                ...
                <Popover.Close key={status} asChild>
                  <button
                    ...
                    style={{
                      ...
                      color: isSelected ? m.color : '#1A1A2E',
                    }}
                    onMouseEnter={(e) =>
                      ((e.currentTarget as HTMLElement).style.background = m.bgColor + '80')
                    }
                    onMouseLeave={(e) =>
                      ((e.currentTarget as HTMLElement).style.background = isSelected ? m.bgColor : 'transparent')
                    }
                  >
                    <span
                      className="w-2 h-2 rounded-full flex-shrink-0"
                    ...
                    {isSelected && <Check size={12} style={{ color: m.color }} />}
                  </button>
                </Popover.Close>
              )
            })}
...
const PRIORITY_ICONS: Record<IssuePriority, React.ReactNode> = {
  urgent: (
    <span className="w-3 h-3 rounded-sm flex-shrink-0 inline-block" style={{ background: '#EF4444' }} />
  ),
  high: (
    <span className="w-3 h-3 rounded-sm flex-shrink-0 inline-block" style={{ background: '#F59E0B' }} />
  ),
  medium: (
    <span className="w-3 h-3 rounded-sm flex-shrink-0 inline-block" style={{ background: '#3B82F6' }} />
  ),
  low: (
    <span className="w-3 h-3 rounded-sm flex-shrink-0 inline-block" style={{ background: '#A0A0B8' }} />
  ),
  no_priority: (
    <span
    ...
    <Popover.Root>
      <Popover.Trigger asChild>
        ...
        >
          ...
          {!compact && <span className="font-medium text-xs">{meta.label}</span>}
        </button>
      </Popover.Trigger>
              ...
              return (
                <Popover.Close key={priority} asChild>
                  <button
                    ...
                    className="w-full flex items-center gap-2.5 px-3 py-2 text-sm transition-colors"
                    style={{
                      background: isSelected ? '#F8F8FC' : 'transparent',
                      color: isSelected ? m.color : '#1A1A2E',
                    }}
                    onMouseEnter={(e) =>
                    ...
                    onMouseLeave={(e) =>
                      ((e.currentTarget as HTMLElement).style.background = isSelected ? '#F8F8FC' : 'transparent')
                    }
                  >
...
export function AssigneePicker({ value, users, onChange, compact = false }: AssigneePickerProps) {
  ...
  return (
    <Popover.Root>
      <Popover.Trigger asChild>
        <button
          className={cn(
            'flex items-center gap-1.5 rounded-lg transition-colors hover:bg-[#F8F8FC]',
            compact ? 'p-1' : 'px-2.5 py-1.5 text-xs'
          )}
        ...
        >
          {assignee ? (
            ...
            <>
              ...
              />
              {!compact && <span style={{ color: '#A0A0B8' }}>Unassigned</span>}
            </>
          )}
        </button>
      </Popover.Trigger>
          ...
          >
            ...
            <Popover.Close asChild>
              <button
                ...
                className="w-full flex items-center gap-2.5 px-3 py-2 text-sm transition-colors"
                style={{
                  background: !value ? '#F8F8FC' : 'transparent',
                  color: '#6B6B8A',
                }}
                onMouseEnter={(e) =>
                  ((e.currentTarget as HTMLElement).style.background = '#F8F8FC')
                ...
                onMouseLeave={(e) =>
                  ((e.currentTarget as HTMLElement).style.background = !value ? '#F8F8FC' : 'transparent')
                }
              >
                <div
                  className="w-5 h-5 rounded-full border-2 border-dashed flex-shrink-0"
