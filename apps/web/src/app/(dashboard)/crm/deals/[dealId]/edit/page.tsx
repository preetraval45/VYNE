...
import { STAGES, SOURCES, ASSIGNEES, type Stage, type Source } from "@/lib/fixtures/crm";
...
export default function EditDealPage() {
  ...
  if (!deal) {
    return (
      <div style={{ padding: 40, textAlign: "center" }}>
        <p style={{ color: "var(--text-secondary)", fontSize: 14, marginBottom: 16 }}>Deal not found.</p>
        <Link
          href="/crm"
          style={{ display: "inline-block", padding: "8px 14px", borderRadius: 8, background: "var(--vyne-purple)", color: "#fff", fontSize: 13, fontWeight: 600 }}
        >
          Back to CRM
        </Link>
      </div>
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    updateDeal(dealId, {
      ...
      probability: Math.max(0, Math.min(100, Number.parseInt(form.probability, 10) || 0)),
      stage: form.stage,
      source: form.source,
      ...
      <form id="edit-deal-form" onSubmit={handleSubmit}>
        <FormSection title="Company">
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
            <FormField label="Contact name" htmlFor="edit-deal-contact" required>
              <input
        ...
        <FormSection title="Pipeline">
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
            ...
            <FormField label="Probability" htmlFor="edit-deal-probability" hint="0–100">
              <input
