...
import { ArrowLeft, Pencil, Trash2, Building2, Mail, User, DollarSign, Target, Tag, Clock } from "lucide-react";
...
export default function DealDetailPage() {
  ...
  function handleDelete() {
    if (!confirm(`Delete deal "${deal!.company}"? This cannot be undone.`)) return;
    deleteDeal(deal!.id);
  ...
  const fields: { icon: typeof Building2; label: string; value: string }[] = [
    ...
    { icon: Clock, label: "Last activity", value: new Date(deal.lastActivity).toLocaleDateString() },
  ];
    ...
    <div className="flex flex-col h-full" style={{ background: "var(--content-bg-secondary)" }}>
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, color: "var(--text-tertiary)", marginBottom: 10 }}>
          <Link href="/crm" style={{ display: "inline-flex", alignItems: "center", gap: 4, color: "var(--text-secondary)" }}>
            <ArrowLeft size={13} /> Back
          </Link>
          <span style={{ width: 1, height: 12, background: "var(--content-border)", margin: "0 6px" }} />
          <Link href="/crm" style={{ color: "var(--text-secondary)" }}>CRM</Link>
          <span style={{ margin: "0 4px" }}>›</span>
          <span style={{ color: "var(--text-primary)", fontWeight: 500 }}>{deal.company}</span>
        </div>

        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16 }}>
          <div>
            <h1 style={{ fontSize: 22, fontWeight: 700, letterSpacing: "-0.02em", color: "var(--text-primary)", lineHeight: 1.2, marginBottom: 8 }}>
              {deal.company}
            </h1>
            <span
              style={{
        ...
        <div style={{ maxWidth: 960, margin: "0 auto", display: "grid", gridTemplateColumns: "1fr 320px", gap: 24, alignItems: "start" }}>
          ...
          <div>
            ...
            >
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                {fields.map((f) => (
                  <div key={f.label}>
                    <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11, fontWeight: 600, color: "var(--text-tertiary)", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 4 }}>
                      <f.icon size={11} />
                      {f.label}
                    </div>
                    <div style={{ fontSize: 14, color: "var(--text-primary)", letterSpacing: "-0.005em" }}>{f.value}</div>
                  </div>
                ))}
              </div>
            </section>
            ...
            {deal.notes && (
              ...
              >
                <h2 style={{ fontSize: 14, fontWeight: 600, color: "var(--text-primary)", marginBottom: 12, letterSpacing: "-0.01em" }}>
                  Notes
                </h2>
                <p style={{ fontSize: 13.5, lineHeight: 1.6, color: "var(--text-secondary)", whiteSpace: "pre-wrap", letterSpacing: "-0.005em" }}>
                  {deal.notes}
                </p>
              </section>
            )}
          ...
          <aside style={{ position: "sticky", top: 24 }}>
            ...
            >
              ...
              <div style={{ fontSize: 28, fontWeight: 700, color: "var(--text-primary)", letterSpacing: "-0.02em", marginBottom: 6 }}>
              ...
              <h3 style={{ fontSize: 11, fontWeight: 600, color: "var(--text-tertiary)", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 6 }}>
                Weighted value
              </h3>
              <div style={{ fontSize: 18, fontWeight: 600, color: "var(--vyne-purple)" }}>
