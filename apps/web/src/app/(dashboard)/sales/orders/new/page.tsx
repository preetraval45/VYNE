...
export default function NewSalesOrderPage() {
  ...
  const dirty = customer.length > 0 || lineItems.some((li) => li.productName || li.unitPrice > 0);
  const validItems = lineItems.filter((li) => li.productName.trim());
  const canSubmit = customer.trim().length > 0 && validItems.length > 0;
  const subtotal = validItems.reduce((s, li) => s + li.quantity * li.unitPrice, 0);
  ...
  return (
    <FormPageLayout
      ...
      subtitle="Create a confirmed sales order"
      breadcrumbs={[{ label: "Sales", href: "/sales" }, { label: "New order" }]}
      backHref="/sales"
      dirty={dirty}
      footer={
        <FormFooterButtons
      ...
      aside={
        <div style={{ background: "var(--content-bg)", border: "1px solid var(--content-border)", borderRadius: 14, padding: 20 }}>
          ...
          <div style={{ fontSize: 13, color: "var(--text-secondary)", marginBottom: 4 }}>{customer || "No customer"}</div>
    ...
    >
      <form id="new-order-form" onSubmit={handleSubmit}>
        <FormSection title="Customer">
          <FormField label="Customer name" htmlFor="order-customer" required>
            <input
              id="order-customer"
              type="text"
              value={customer}
        ...
        <FormSection title="Line items" description="At least one item required.">
