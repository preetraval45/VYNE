...
async function forwardToFormspree(email: string, meta: Record<string, string | undefined>) {
...
export async function POST(request: Request) {
  ...
  const [formspree, resend] = await Promise.all([
    forwardToFormspree(email, meta).catch(() => ({ ok: false, skipped: false })),
    forwardToResend(email).catch(() => ({ ok: false, skipped: false })),
  ])

  const anyAttempted = !formspree.skipped || !resend.skipped
  const anySucceeded = (formspree.ok && !formspree.skipped) || (resend.ok && !resend.skipped)
