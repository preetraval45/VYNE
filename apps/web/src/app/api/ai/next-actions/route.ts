...
function fallback(payload: NextActionsPayload): NextActionsResponse {
  ...
  if (status.includes('todo') || status.includes('backlog')) {
    acts.push({
      id: 'a1',
      type: 'assign',
      label: payload.assignee ? 'Confirm assignee is available' : 'Assign an owner',
      rationale:
    ...
    acts.push({
      ...
      rationale: 'Clear "done" definition prevents scope creep before work begins.',
    })
  }
  ...
  if ((payload.priority ?? '').toLowerCase().includes('urgent')) {
    acts.push({
      ...
      rationale: 'Urgent flag set — surface visibility beyond the issue thread.',
    })
  }
