...
const DEMO_CORPUS: Array<{
...
}> = [
  {
    ...
    snippet: 'p95 above 5s on /api/ai/agents/query — traced to upstream model provider, 46-min outage.',
    href: '/observe',
  ...
  },
  {
    ...
    snippet: 'Day 1: laptop, accounts, intro to the team. Day 2: read the Master Plan and ship a hotfix.',
    href: '/docs',
  ...
  },
  {
    ...
    snippet: 'Production IAM role missing kms:Decrypt on staging-secrets KMS key. Blocking deploys.',
    href: '/projects',
  ...
  },
  {
    ...
    snippet: 'Root cause: upstream Anthropic model degradation. Impact: 47 stuck orders, ~$12k at risk. Resolved in 46 min.',
    href: '/docs',
  ...
  },
  {
    ...
    snippet: '38 units left of PWR-003 — Vyne AI flagged critical, supplier reorder PO drafted.',
    href: '/ops',
  ...
  },
  {
    ...
    snippet: '17 messages today — most recent: "api-service v2.4.1 deployment failed".',
    href: '/chat',
...
function heuristicSearch(query: string): SearchResponse {
  ...
  const scored = DEMO_CORPUS.map((c) => {
    const haystack = `${c.title} ${c.snippet} ${c.keywords.join(' ')}`.toLowerCase()
    let score = 0
  ...
  return {
    ...
    hits: scored.map((c) => ({
      ...
      reason: `Matched on: ${c.keywords.filter((k) => tokens.some((t) => k.includes(t))).slice(0, 3).join(', ') || 'partial text'}.`,
    })),
  }
}
...
export async function POST(request: Request) {
  ...
  const corpus = body.corpus && body.corpus.length > 0 ? body.corpus : DEMO_CORPUS
